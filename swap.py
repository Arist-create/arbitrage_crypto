...
async def main():
    ...
    while True:
        ...
        tasks = []
        for pair in pairs:
            main_token = tokens.get(pair['symbol'][:-4])
            if not main_token:
                continue
...
async def check_prices(main_token, usdt_token, chains, gas_price, goplus):
    ...
    async with httpx.AsyncClient(
        ...
        for i in main_token['networkList']:
            ...
            goplus_detect = goplus.get(contract_address.lower())
